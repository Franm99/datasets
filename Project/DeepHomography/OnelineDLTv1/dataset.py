...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_tensor = org_img[:, y: y + self.patch_h, x: x + self.patch_w]
        ...
        return (org_img, input_tensor, patch_indices, h4p)
...
class MyDataset(Dataset):
    def __init__(self, data_path, patch_w=560, patch_h=315, rho=16, WIDTH=640, HEIGHT=360):
        ...
        self.work_dir = os.path.join(data_path, 'Data')
        # MyTest is composed of two pair of images only
    ...
    def __getitem__(self, index):
        ...
        # Concatenate images (prepare tensor)
        org_img = np.concatenate([img_1, img_2], axis=0)
        WIDTH  = org_img.shape[2]
        HEIGHT = org_img.shape[1] # org_img.shape[0] = 1 (channel)

        x, y = 0, 0
        # By using patch = complete image -> input_tensor == org_img
        input_tensor = org_img[:, y: y+self.patch_h, x: x+self.patch_w]
        ...
        return (org_img, input_tensor, patch_indices, four_points,print_img_1,
                print_img_2, video_name, npy_id)
...
class MyDataset2(Dataset):
    def __init__(self, data_path, patch_w=560, patch_h=315, rho=16, WIDTH=640, HEIGHT=360):
        ...
        self.work_dir = os.path.join(data_path, 'Data')
        self.pair_list = list(open(os.path.join(self.work_dir, 'Test_List.txt')))
        self.pair_list = [self.pair_list[i] for i in range(len(self.pair_list)) if self.pair_list[i][:7] == '0000016']
        print(len(self.pair_list))
        self.img_path = os.path.join(self.work_dir, 'MyTest2/')
        self.npy_path = os.path.join(self.work_dir, 'Coordinate/')
